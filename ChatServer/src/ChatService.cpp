...
ChatService::ChatService()
{
	...
	using InviteToChatRoomT = std::function<StatusResponse(const Connection::Ptr&, const InviteToChatRoomRequest&, Status&)>;
	...
	RegisterServiceHandler<InviteToChatRoomT, InviteToChatRoomRequest, StatusResponse>(MethodType::InviteToChatRoom, inviteToChatRoomHandler);
	...
	using OneChatHandlerT = std::function<StatusResponse(const Connection::Ptr&, const OneChatRequest&, Status&)>;
	auto oneChatHandler = std::bind(&ChatService::OneChatHandler, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
	RegisterServiceHandler<OneChatHandlerT, StatusResponse, OneChatRequest>(MethodType::OneChat, oneChatHandler);

	using GroupChatHandlerT = std::function<StatusResponse(const Connection::Ptr&, const GroupChatRequest&, Status&)>;
	...
	RegisterServiceHandler<GroupChatHandlerT, StatusResponse, GroupChatRequest>(MethodType::GroupChat, groupChatHandler);

	using AddFrinedHandlerT = std::function<StatusResponse(const Connection::Ptr&, const AddFriendRequest&, Status&)>;
	...
	RegisterServiceHandler<AddFrinedHandlerT, StatusResponse, AddFriendRequest>(MethodType::AddFriend, addFrinedHandler);
...
void ChatService::InitDatabase()
{
	...
	constexpr char* createFriendTable = R"(
            CREATE TABLE IF NOT EXISTS friends (
				...
				INDEX idx_user_status ON friends(user_id, status),
				INDEX idx_friend_status ON friends(friend_id, status)
            );
        )";
	...
	constexpr char* createMessageTable = R"(
            CREATE TABLE IF NOT EXISTS messages (
				...
				INDEX idx_room_receiver ON messages(room_id, sender_id)
            );
        )";
	...
	constexpr char* createOffLineRecpTable = R"(
            CREATE TABLE IF NOT EXISTS offline_recipients (
				...
				INDEX idx_offline_recipient ON offline_recipients(recipient_id)
            );
        )";
	...
	constexpr char* createChatRoomTable = R"(
            CREATE TABLE IF NOT EXISTS chat_rooms (
				...
				INDEX idx_chat_rooms_owner ON chat_rooms(owner_id)
            );
        )";
	...
	constexpr char* createChatRoomMembersTable = R"(
            CREATE TABLE IF NOT EXISTS chat_room_members (
				...
				INDEX idx_room_status ON chat_room_members(room_id, status),
				INDEX idx_user_status ON chat_room_members(user_id, status),
                FOREIGN KEY(room_id) REFERENCES chat_rooms(id)
            );
        )";
	auto createTableSqls = std::initializer_list{ createUserTable,
												 createFriendTable,
...
StatusResponse ChatService::RegisterHandler(const Connection::Ptr& conn, const AuthRequest& request, Status& status)
{
	...
	auto results = p_sqlExecuser->ExecuteQuery<User>(QueryUserById, request.m_username);
	if (results.size() >= 1)
	{
		status = BAD_REQUEST;
...
AuthResponse ChatService::LoginHandler(const Connection::Ptr& conn, const AuthRequest& request, Status& status)
{
	...
	auto friends = p_sqlExecuser->ExecuteQuery<User>(QueryFriendsByUserId, user.m_id);
...
ChatRoomResponse ChatService::CreateChatRoomHandler(const Connection::Ptr& conn, const CreateChatRoomRequest& request, Status& status) {
	...
	try
	{
		...
		p_sqlExecuser->ExecuteUpdate(InsertChatRoomMember, response.m_chatRoom.m_id, request.m_ownerId, ChatRoomRole::OWNER, AddRelationStatus::ACCEPTED);
		p_sqlExecuser->CommitTransaction();
	}
	catch (const std::exception& e)
	{
...
StatusResponse ChatService::InviteToChatRoomHandler(const Connection::Ptr& conn, const InviteToChatRoomRequest& request, Status& status) {
	StatusResponse response;
	...
	try
	{
		auto member = p_sqlExecuser->ExecuteUpdateAndReturn<ChatRoomMember>
			(InsertChatRoomMember, request.m_roomId, request.m_inviterId, ChatRoomRole::MEMBER, AddRelationStatus::PENDING);
		if (auto inviteeConn = GetUserConnection(request.m_inviteeId))
		{
			ChatRoomInvitation invitation;
			invitation.m_id = member.m_id;
			{
				std::lock_guard<std::mutex> lock(m_userMtx);
				invitation.m_inviter = m_userMap[request.m_inviterId];
			}
			invitation.m_chatRoom = p_sqlExecuser->ExecuteQuery<ChatRoom>(QueryChatRoomByCrId, member.m_roomId)[0];
			PushOnlineData<ChatRoomInvitation>(inviteeConn, invitation, PushType::ChatRoomInvitation);
		}
		p_sqlExecuser->CommitTransaction();
	}
	catch (const std::exception& e)
...
ChatRoomResponse ChatService::AcceptChatRoomtInvitationHandler(const Connection::Ptr& conn, const HandleInvitationRequest& request, Status& status) {
	...
	auto results = p_sqlExecuser->ExecuteQuery<ChatRoomMember>(QueryChatRoomMemberByIdAndUId, request.m_invitation_id, request.m_userId);
	if (results.size() < 1) {
		status = BAD_REQUEST;
	...
	if (invitation.m_status != AddRelationStatus::PENDING) {
		status = BAD_REQUEST;
		response.m_error_msg = "Invitation has been handled!";
		return response;
	}
	p_sqlExecuser->ExecuteUpdate(UpdateChatRoomMemberStatus, AddRelationStatus::ACCEPTED, request.m_invitation_id);
	response.m_chatRoom = p_sqlExecuser->ExecuteQuery<ChatRoom>(QueryChatRoomByCrId, invitation.m_roomId)[0];
	if (auto userConn = GetUserConnection(request.m_userId))
	{
		FriendInvitationDecision decision;
		decision.m_invitationId = request.m_invitation_id;
		{
			std::lock_guard<std::mutex> lock(m_userMtx);
			decision.m_user = m_userMap[request.m_userId];
		}
		decision.m_status = AddRelationStatus::ACCEPTED;
		PushOnlineData<FriendInvitationDecision>(userConn, decision, PushType::ChatRoomInvitationDecision);
	}
	status = SUCCESS;
	return response;
}
...
StatusResponse ChatService::RejectChatRoomInvitationHandler(const Connection::Ptr& conn, const HandleInvitationRequest& request, Status& status) {
	...
	if (results[0].m_status != AddRelationStatus::PENDING) {
		status = BAD_REQUEST;
		response.m_error_msg = "Invitation has been handled!";
		return response;
	}
	...
	if (auto userConn = GetUserConnection(request.m_userId))
	{
		...
		PushOnlineData<StatusResponse>(userConn, response, PushType::ChatRoomInvitationDecision);
	}
	status = SUCCESS;
	return response;
}

StatusResponse ChatService::OneChatHandler(const Connection::Ptr& conn, const OneChatRequest& request, Status& status) {
	StatusResponse response;
	...
	try
	{
		auto message = p_sqlExecuser->ExecuteUpdateAndReturn<Message>(InsertMessage, nullptr, request.m_senderId, request.m_message);
		if (auto receiverConn = GetUserConnection(request.m_receiverId))
		{
			PushOnlineData<Message>(receiverConn, message, PushType::Message);
		}
		else {
			p_sqlExecuser->ExecuteUpdate(InsertOfflineRecipient, message.m_id, request.m_receiverId);
		}
		p_sqlExecuser->CommitTransaction();
	}
	catch (const std::exception& e)
...
StatusResponse ChatService::GroupChatHandler(const Connection::Ptr& conn, const GroupChatRequest& request, Status& status) {
	StatusResponse response;
	if (!CheckAuth(request.m_senderId, conn)) {
		status = UNAUTHORIZED;
		response.m_error_msg = "Unauthorized access!";
	...
	try
	{
		...
		for (const auto& member : members) {
			...
			if (auto receiverConn = GetUserConnection(member.m_userId))
			{
				...
				p_sqlExecuser->ExecuteUpdate(InsertOfflineRecipient, message.m_id, member.m_userId);
			}
		}
		if (isUserInChatRoom)
		{
...
StatusResponse ChatService::AddFriendHandler(const Connection::Ptr& conn, const AddFriendRequest& request, Status& status) {
	StatusResponse response;
	if (!CheckAuth(request.m_userId, conn)) {
		status = UNAUTHORIZED;
		response.m_error_msg = "Unauthorized access!";
	...
	auto results = p_sqlExecuser->ExecuteQuery<User>(QueryFriendsByUserIdAndFrdId, request.m_userId, request.m_friendId);
	if (results.size() > 0) {
		status = BAD_REQUEST;
	...
	if (auto friendConn = GetUserConnection(request.m_friendId)) {
		...
		invitation.m_id = friendRlt.m_id;
		{
			std::lock_guard<std::mutex> lock(m_userMtx);
			invitation.m_inviter = m_userMap[request.m_userId];
		}
		PushOnlineData<FriendInvitation>(friendConn, invitation, PushType::FriendInvitation);
	}
	status = SUCCESS;
	return response;
}
...
UserResponse ChatService::AcceptFriendHandler(const Connection::Ptr& conn, const HandleInvitationRequest& request, Status& status) {
	...
	auto results = p_sqlExecuser->ExecuteQuery<FriendRelation>(QueryChatRoomMemberByIdAndUId, request.m_invitation_id, request.m_userId);
	if (results.size() < 1) {
		status = BAD_REQUEST;
		response.m_error_msg = "Invitation not found!";
		return response;
	...
	response.m_user = p_sqlExecuser->ExecuteQuery<User>(QueryUserById, request.m_userId)[0];
	if (auto userConn = GetUserConnection(request.m_userId))
	{
		FriendInvitationDecision decision;
		decision.m_invitationId = request.m_invitation_id;
